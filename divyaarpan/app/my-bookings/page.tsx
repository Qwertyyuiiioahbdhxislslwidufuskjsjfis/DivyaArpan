    ...
    <main className="min-h-screen bg-orange-50 px-6 py-12 md:py-16">
      <section className="mx-auto max-w-6xl">
        ...
        {!hasBookings ? <section className="rounded-2xl bg-white p-12 text-center shadow-lg"><div className="text-6xl">🙏</div><h2 className="mt-5 text-3xl font-bold text-orange-700">No Bookings Yet</h2><p className="mt-3 text-gray-600">Your confirmed and pending bookings will appear here.</p></section> : <div className="space-y-8">
          {bookings.temple.map((booking) => <BookingCard key={booking.bookingId} kind="temple" title={booking.temple} subtitle={booking.pooja} bookingId={booking.bookingId} status={booking.status} paymentStatus={booking.paymentStatus} amount={formatAmount(booking.amount, booking.price)} date={booking.date} time={booking.time} details={[`Mode: ${booking.poojaMode === "ON_BEHALF" ? "Arranged on behalf" : "Devotee present"}`, `Duration: ${booking.duration}`, `Devotees: ${booking.devotees}`]} sankalp={booking.sankalp} statusHistory={booking.statusHistory} />)}
